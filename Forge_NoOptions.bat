...
if not exist %~dp0stable-diffusion-webui-forge\venv\ (
	echo call %~dp0EasyReforge\SetupForge.bat
	call %~dp0EasyReforge\SetupForge.bat
)
if not exist %~dp0stable-diffusion-webui-forge\venv\ (
	echo "[Error] %~dp0stable-diffusion-webui-forge\venv\ が見つかりません。"
	pause & exit /b 1
)

pushd %~dp0stable-diffusion-webui-forge

echo call %~dp0EasyReforge\Forge\ForgeConfig.bat config.json
call %~dp0EasyReforge\Forge\ForgeConfig.bat config.json
if %ERRORLEVEL% neq 0 ( popd & exit /b 1 )

echo call %~dp0EasyReforge\Forge\ForgeUiConfig.bat ui-config.json
call %~dp0EasyReforge\Forge\ForgeUiConfig.bat ui-config.json
if %ERRORLEVEL% neq 0 ( popd & exit /b 1 )
